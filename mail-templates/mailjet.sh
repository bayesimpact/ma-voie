...
readonly JSON_MAIL_SELECTOR='.Data | .[0]'
...
readonly TEMPLATES_FOLDER="$(realpath "$(dirname ${BASH_SOURCE})")"

grep="grep"
if ! echo 'abc' | grep -qP '(?<=a)bc' 2>/dev/null; then
  if ! echo 'abc' | ggrep -qP '(?<=a)bc' 2>/dev/null; then
    if [ "$(uname)" == "Darwin" ] && [ -x "$(which brew)" ]; then
      echo "Please, install GNU grep through brew: \`brew install grep\`"
    else
      echo "Unable to grep PERL regexes, please find a way to make \`grep -P\` work."
    fi
    exit 1
  fi
  grep="ggrep"
...
for template in $(jq -c .[] "$TEMPLATES_FOLDER/index.json"); do
