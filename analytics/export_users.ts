...
const fields = {
  ...
  // Steps data
  ...Object.fromEntries(steps.map((step: bayes.maVoie.StepId) => [`${step}Partner`, (user: unknown, data: User): string => data.projects?.[0].steps?.[step]?.selectedPartnerId || ''])),
  ...Object.fromEntries(steps.map((step: bayes.maVoie.StepId) => [`${step}Completed`, (user: unknown, data: User): string => data.projects?.[0].steps?.[step]?.completed || ''])),
}
