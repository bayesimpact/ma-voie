...
describe('Generic rule', () => {
  ...
  before(() => {
    app = firebase.initializeTestApp({
      // Most of the operations require an authentified user.
      auth: {
        email: 'alice@example.com',
        uid: 'aliceUserId',
      },
      // We need a project ID to set the emulator up.
...
describe('User rule', () => {
  ...
  it('should allow to create a user with the auth ID', () =>
    expect(app.firestore().collection('users').doc('aliceUserId').set({
