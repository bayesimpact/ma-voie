...
import {useFeedback} from 'store/selections'
...
const Layout = ({bigTitle, children, header, menu, style, title}: Props): React.ReactElement => {
  const hasFeedbackAlready = useFeedback()
  ...
  const layoutWithoutMenuStyle: React.CSSProperties = isMenuAlwaysShown ? {
    ...
    paddingBottom: hasFeedbackAlready ? 0 : 250,
    position: 'relative',
  }
  const layoutStyle = {
    ...containerStyle,
  ...
  const mainContent = <React.Fragment>
    ...
    <div style={layoutWithoutMenuStyle}>
      ...
      {hasFeedbackAlready ? null : <Feedback />}
    </div>
  </React.Fragment>
  ...
  const divStyle = {
    paddingBottom: hasFeedbackAlready ? 0 : 150,
    ...withMenuStyle,
  }
