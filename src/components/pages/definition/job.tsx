...
const JobPage = (): React.ReactElement => {
  ...
  const title = prepareT('Pour quel métier souhaitez-vous retrouver un poste\u00A0?')
  ...
  return <Layout header={t('Définition')} title={title}>
    ...
    <Link to={getPath('DEFINITION_LOST', t)} style={linkStyle}>
      <Button color={colors.DARK_FOREST_GREEN} style={buttonStyle}>{t('Je ne sais pas')}</Button>
    </Link>
  </Layout>
}
